...
class MultiChoiceReadingListAPIView(ListAPIView):
    queryset = MultiChoiceReading.objects.filter(single=False)
    serializer_class = MultiChoiceReadingListSerializer
    pagination_class = CustomPagination

class MultiChoiceReadingSingleanswerListAPIView(ListAPIView):
    queryset = MultiChoiceReading.objects.filter(single=True)
    serializer_class = MultiChoiceReadingListSerializer
    pagination_class = CustomPagination

class MultiChoiceReadingCreateAPIView(CreateAPIView):
    permission_classes = [IsAdminUser]
...
class MultiChoiceReadingDetailsView(RetrieveAPIView):
    lookup_field = "pk"
    serializer_class = MultiChoiceReadingDetailsSerializer
    queryset = MultiChoiceReading.objects.all()

class MultiChoiceReadingAnswerCreateView(APIView):
    permission_classes = [IsStudentPermission]
    ...
    def post(self, request):
        serializer = MultiChoiceReadingAnswerCreateSerializer(data=request.data)
        if serializer.is_valid():
            score = self.calculate_score(serializer.validated_data['answers'], serializer.validated_data['multi_choice'].right_options)
            serializer.save(user=self.request.user, score=score)
            return Response({
                ...
                "score": score,
                "right_options": serializer.validated_data['multi_choice'].right_options,
                "wrong_answers": [answer for answer in serializer.validated_data['answers'] if answer not in serializer.validated_data['multi_choice_reading'].right_options],
                "max_score": len(serializer.validated_data['multi_choice_reading'].right_options)
            })
        else:
            return Response(serializer.errors, status=status.HTTP_422_UNPROCESSABLE_ENTITY)

class MultiChoiceReadingAnswerListView(ListAPIView):
    serializer_class = MultiChoiceReadingAnswerListSerializer
    def get_queryset(self):
        pk = self.kwargs.get('pk')
        queryset = Answer.objects.filter(multi_choice_reading=pk)

        return queryset

class MyReadingAnswerListView(ListAPIView):
    serializer_class = MultiChoiceReadingAnswerListSerializer
    permission_classes = (IsStudentPermission,)
    def get_queryset(self):
        # Get the primary key (pk) from the URL query parameters
        ...
        # Filter the Answer objects based on the 'summarize' field with the given 'pk'
        queryset = Answer.objects.filter(multi_choice_reading=pk, user=self.request.user)
